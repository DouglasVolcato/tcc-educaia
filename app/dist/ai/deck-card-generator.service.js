import { LlmAgent, MessageSenderEnum } from "./agent.js";
import { FlashcardFormatterTool } from "./tools/flashcard-formatter.tool.js";
export class DeckCardGeneratorService {
    constructor(agentFactory = () => new LlmAgent()) {
    ...
    async generateCards(input) {
        ...
        agent.addMessage({
            ...
            content: `Você é um especialista em educação que cria flashcards estilo pergunta e resposta para revisão espaçada. Foque em informações importantes, contexto claro e linguagem em português brasileiro.
        ...
        const toolCall = response.toolCalls.find((call) => call.name === FlashcardFormatterTool.NAME);
        if (!toolCall) {
