...
class RepositoryStup extends Repository {
    constructor(tableName, idField, publicFields, insertFields, updateFields) {
        super({
            tableName,
            idField,
            publicFields,
...
const makeSut = (tableName, idField, publicFields, insertFields, updateFields) => {
    const sut = new RepositoryStup(tableName, idField, publicFields, insertFields, updateFields);
    return { sut };
};
describe("Repository", () => {
    const dbQueryMock = DbConnection.query;
    beforeEach(() => {
        jest.clearAllMocks();
    });
    describe("Constructor", () => {
        test("Should create a new repository with given properties", () => {
            ...
            expect(sut.idField).toBe(idField);
            expect(sut.publicFields).toEqual(publicFields);
            expect(sut.insertFields).toEqual(insertFields);
            expect(sut.updateFields).toEqual(updateFields);
        });
    });
    describe("Insert", () => {
        test("Should call connection query with right values", async () => {
            ...
            const expectedParams = insertFields.map((fieldName) => fields.find((field) => field.key === fieldName).value);
        ...
        test("Should throw if extractFieldsFromObject throws", async () => {
            ...
            const error = new Error(FakeData.phrase());
            const spy = jest
                .spyOn(sut, "extractFieldsFromObject")
                .mockImplementationOnce(() => {
                throw error;
            });
            await expect(sut.insert({ fields: [{ key: insertFields[0], value: FakeData.word() }] })).rejects.toThrow(error);
            spy.mockRestore();
        });
    });
    describe("Update", () => {
    ...
    `;
        ...
        test("Should throw if connection query throws", async () => {
            ...
            const error = new Error(FakeData.phrase());
            dbQueryMock.mockRejectedValueOnce(error);
            await expect(sut.findMany({ limit: 1, offset: 0, params: [{ key: FakeData.word(), value: FakeData.word() }] })).rejects.toThrow(error);
        });
    ...
    describe("ExecuteSql", () => {
        ...
        test("Should throw if connection query throws", async () => {
            ...
            const error = new Error(FakeData.phrase());
            dbQueryMock.mockRejectedValueOnce(error);
            await expect(sut.executeSql({ query: FakeData.phrase(), params: [FakeData.word()] })).rejects.toThrow(error);
        });
        test("Should return what connection query returns", async () => {
            const tableName = FakeData.word();
            const idField = FakeData.word();
